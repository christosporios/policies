...
export const PolicyViewer = ({ policyEntry, onBack }) => {
  const { slug, data: policy, raw: policyYaml, error: yamlError } = policyEntry;
  ...
  useEffect(() => {
    if (!mobile) { setShowStickyHeader(false); return; }
  ...
  if (!refIndexRef.current && policy) refIndexRef.current = makeRefIndex(policy.references);
    ...
    <div style={{ fontFamily: C.sans, background: C.bg, color: C.ink, fontSize: 15, lineHeight: 1.7, WebkitFontSmoothing: 'antialiased', minHeight: '100vh' }}>
      ...
      {mobile && (
        <div style={{
          ...
          <span onClick={onBack} style={{ cursor: 'pointer', opacity: 0.5 }}>&larr;</span>
          {policy.meta.title}
        </div>
      )}
      ...
      <header ref={headerRef} style={{ background: C.ink, color: '#f7f6f4', padding: mobile ? '40px 0 36px' : '64px 0 56px', position: 'relative', overflow: 'hidden' }}>
        ...
        <motion.div
          initial={{ opacity: 0 }}
          animate={{ opacity: 1 }}
          transition={{ duration: 0.35, delay: onBack ? 0.2 : 0 }}
        ...
        >
          ...
          <motion.h1
            layoutId={onBack ? `policy-title-${slug}` : undefined}
            transition={{ layout: { duration: 0.5, ease: [0.4, 0, 0.2, 1] } }}
            style={{ fontFamily: C.serif, fontSize: mobile ? 28 : 'clamp(32px, 5vw, 50px)', fontWeight: 700, lineHeight: 1.2, letterSpacing: '-0.02em', color: '#f7f6f4', marginBottom: 0 }}
          >
            {policy.meta.title}
          </motion.h1>
